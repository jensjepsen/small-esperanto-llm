#!/usr/bin/env bash
# Danish SFT v22 (mix13, 3 epochs, constant LR 3e-5) — v16 + wiki-mc-letters.
#
# v16 mix + jensjepsen/danish-wiki-mc-letters-v1 (210k MC rows built from
# wiki-closedqa + stem to teach letter/digit label emission — targets the
# positional-bias failure mode on cit-MC that's been present since v4).
#
# Recipe otherwise identical to v16:
#   - 3 epochs, constant_with_warmup LR 3e-5, warmup 200
#   - Full-set downstream evals every 0.25 epoch
#   - Top-3 preservation for later averaging (avg-top3 is best v16 model
#     so we expect the same pattern here — see project-sft-ckpt-averaging)
#
# Runtime ~5h on 5090 (13 sources ~1.57M rows × 3 epochs).

set -euo pipefail
cd /root/espllm

export WANDB_PROJECT=danish-lm-sft
export WANDB_API_KEY=$(grep -m1 password ~/.netrc | awk '{print $2}')
export ESPLLM_NUM_PROC=8

uv run python -u scripts/train_sft_packed.py \
  --checkpoint jensjepsen/danish-lm-400m-base-ckpt310k \
  --tokenizer jensjepsen/danish-tokenizer \
  --output-dir /root/runs/sft/da_v22_mix13_mc_letters_3e \
  --no-morpheme-preprocess \
  --sft-data \
    jensjepsen/danish-metamath-gsm:sft \
    jensjepsen/danish-algebra-sft-v5-mixed \
    jensjepsen/danish-arith-chain-sft-v1 \
    jensjepsen/danish-wiki-grounded-sft-v3:sft \
    jensjepsen/danish-text-to-question-v2:sft \
    jensjepsen/danish-sciq:sft:train \
    jensjepsen/danish-gsm8k:sft:train \
    jensjepsen/danish-instruction-following-v3:sft:train \
    jensjepsen/danish-wiki-closedqa-v1:sft \
    jensjepsen/danish-word-problems-v2 \
    jensjepsen/danish-wiki-closedqa-stem-v1:sft \
    jensjepsen/danish-wiki-broadqa-stem-v1:sft \
    jensjepsen/danish-wiki-mc-letters-v1 \
  --epochs 3 \
  --batch-size 32 \
  --gradient-accumulation 1 \
  --learning-rate 3e-5 \
  --max-length 512 \
  --lr-scheduler constant_with_warmup \
  --warmup-steps 200 \
  --save-fraction-of-epoch 0.25 \
  --save-total-limit 2 \
  --downstream-evals gsm8k sciq citgen \
  --downstream-batch-size 32 \
  --top-k-downstream 3 \
  --wandb-project danish-lm-sft \
  --wandb-run-name da_v22_sft_mix13_mc_letters_3e \
  --wandb-tags sft da v22 mix13 no-morpheme mc-letters constant-lr epochs-3 downstream-eval full-set top3
