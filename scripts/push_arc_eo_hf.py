"""Push Esperanto-translated AI2 ARC (Challenge + Easy) to HF Hub.

Flat schema, one config per subset (arc_challenge, arc_easy), preserving
official train/validation/test splits. Rows carry both EO and EN fields
following the esperanto-sciq / esperanto-piqa pattern.

Row schema:
    id             str   — original ARC row id (e.g. Mercury_SC_401336)
    question       str   — EO
    choices        list[dict{label, text}]  — EO
    answerKey      str   — original ARC gold letter (A/B/C/D or 1/2/3/4)
    en_question    str
    en_choices     list[dict{label, text}]

Usage:
    uv run python scripts/push_arc_eo_hf.py \\
        --input /mnt/data2/arc_eo.jsonl \\
        --repo jensjepsen/esperanto-arc
"""
    ...
    with path.open() as f:
        for line in f:
            ...
            if r.get("reject_reason"):
                continue
            if not r.get("eo_question") or not r.get("eo_choices"):
                continue
            row = {
                "id": r["id"],
                "question": r["eo_question"],
                "choices": r["eo_choices"],
...
def main():
    ...
    ap.add_argument("--input", type=Path, default=Path("/mnt/data2/arc_eo.jsonl"))
    ap.add_argument("--repo", default="jensjepsen/esperanto-arc")
    args = ap.parse_args()
