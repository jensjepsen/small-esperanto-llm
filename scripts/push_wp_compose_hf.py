"""Push the wp_compose v2 dataset to HF Hub.

Two configs (matches jensjepsen/esperanto-word-problems v1 layout):
  - default: full schema {question, answer, chain_lines, final, recipe, n_steps}
  - sft:     messages format for direct SFT consumption

Usage:
  HF_HUB_TOKEN=$(cat ~/.cache/huggingface/token) \\
  uv run python scripts/push_wp_compose_hf.py \\
    --input /mnt/data2/wp_compose_v1_150k.jsonl \\
    --repo jensjepsen/esperanto-word-problems-v2
"""
import argparse
import json
import os
